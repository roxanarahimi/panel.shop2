...
//                dd($cart);
            ...
            $item = OrderItem::where('product_id', $request['product_id'])->where('product_size_id', $request['product_size_id'])->where('order_id', $cart['id'])->first();
            if ($item) {
                $q = $item['quantity'] + 1;
...
//                }
    ...
    public function userToGateway(Order $order)
    {

...
//        Redis::set($request['mobile'], Random::generate(4, '0-9'));
        ...
        if ($user) {
            ...
            if (($user['scope'] == 'user' && $request['scope'] == 'user')) {

                $xo = KeyValue::where('key', $request->mobile)->first();
                if ($xo) {
                    $xo->update(['value' => Random::generate(4, '0-9')]);
                } else {
                    $xo = KeyValue::create(['key' => $request['mobile'], 'value' => Random::generate(4, '0-9')]);
                }
    ...
    public function loginMobile(Request $request)
    {
        try {
            $user = User::where('mobile', $request->mobile)->first();
            $code = KeyValue::where('key', $request->mobile)->first();
